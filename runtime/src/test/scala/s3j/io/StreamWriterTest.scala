...
class StreamWriterTest extends AnyFlatSpec with Matchers {
  ...
  it should "serialize nested arrays/objects" in {
    generateJson(0, _.beginObject().key("x").beginObject().end().key("y").beginObject().key("z")
      .boolValue(true).end().end()) shouldBe "{\"x\":{},\"y\":{\"z\":true}}"

    generateJson(2, _.beginObject().key("x").beginObject().end().key("y").beginObject().key("z")
      .boolValue(true).end().end()) shouldBe "{\n  \"x\": {},\n  \"y\": {\n    \"z\": true\n  }\n}"

    generateJson(0, _.beginArray().beginArray().end().beginArray().boolValue(true).end().end()) shouldBe
      "[[],[true]]"

    generateJson(2, _.beginArray().beginArray().end().beginArray().boolValue(true).end().end()) shouldBe
      "[\n  [],\n  [\n    true\n  ]\n]"
  }
  ...
  it should "serialize escaped strings" in {
    generateJson(0, _.beginObject().key("\u0000\uDDEE").stringValue("\uFFFF").end()) shouldBe
      "{\"\\u0000\\uDDEE\":\"\\uFFFF\"}"

    generateJson(0, _.stringValue("\r\t\n\f\b\\\"")) shouldBe "\"\\r\\t\\n\\f\\b\\\\\\\"\""
  }
}
