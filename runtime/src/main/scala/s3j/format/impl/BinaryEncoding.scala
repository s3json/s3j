...
private[impl] abstract class GenericBase64 extends BinaryEncoding {
  ...
  def encode(n: Int, in: Array[Byte], inOffset: Int, out: Array[Char]): Int = {
    ...
    while (dst < dstEnd) {
      ...
      out(dst) = alpha(b1 >> 2);
      dst += 1
      out(dst) = alpha(((b1 << 4) | (b2 >> 4)) & 0x3F);
      dst += 1
      out(dst) = alpha(((b2 << 2) | (b3 >> 6)) & 0x3F);
      dst += 1
      out(dst) = alpha(b3 & 0x3F);
      dst += 1
    }
  ...
  def decode(n: Int, in: Array[Char], inOffset: Int, out: Array[Byte]): Int = {
    ...
    while (src < srcEnd) {
      val c1 = decodeChar(in(src));
      src += 1
      val c2 = decodeChar(in(src));
      src += 1
      val c3 = decodeChar(in(src));
      src += 1
      val c4 = decodeChar(in(src));
      src += 1

      out(dst) = ((c1 << 2) | (c2 >> 4)).toByte;
      dst += 1
      out(dst) = ((c2 << 4) | (c3 >> 2)).toByte;
      dst += 1
      out(dst) = ((c3 << 6) | c4).toByte;
      dst += 1
    }
...
private[impl] abstract class GenericHex extends BinaryEncoding {
  protected def alphabet: Array[Char]
  ...
  def decode(n: Int, in: Array[Char], inOffset: Int, out: Array[Byte]): Int = {
    ...
    while (dst < n) {
      val b1 = decodeChar(in(src));
      src += 1
      val b2 = decodeChar(in(src));
      src += 1
      out(dst) = ((b1 << 4) | b2).toByte;
      dst += 1
    }
